...
cpr_pat <- obs_data_filter %>%
  filter( concept_code == "CPR")
```

## Add vaccination status
```{r}
vaccinated_pat <- obs_raw %>%
  filter( patient_num %in% misc_hosp_length$patient_num ) %>%
  filter( concept_code == "CovidVaccine") %>%
  select( patient_num, days_since_admission, concept_type, concept_code)

## at BCH only 4 patients with the CovidVaccine code and none of them part of the misc cohort
```
...
mainTable <- misc_hosp_length %>%
  ...
  mutate( ecmo = ifelse( patient_num %in% ecmo_pat$patient_num, 1, 0), 
          cpr = ifelse( patient_num %in% cpr_pat$patient_num, 1, 0), 
          vaccinated = ifelse( patient_num %in% vaccinated_pat$patient_num, 1, 0 ))
...
mainTable <- mainTable %>%
  left_join( clin_var2merge, by = "patient_num") %>%
  replace(is.na(.), 0)
```
